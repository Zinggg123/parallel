...
vector<int> lalian2(vector<rbm> b, vector<int> t) {
	//获取各列表的头元素
	vector<vector<int>::const_iterator> iter;
	int minvalue = 0;
	for (int i = 0; i < b.size(); i++) {
		iter.push_back(b[i].buc[t[i]].begin());
		if (*iter[i] > minvalue) {
	...
	while (true) {
		...
		for (int i = 0; i < iter.size(); i++) {
			...
			if (i == iter.size() - 1) {
				...
				result.push_back(*iter[i] + b[i].index[t[i]] * 256);
				++iter[0];//更新一下最小值
				if (iter[0] == b[0].buc[t[i]].end()) {
					finish = true;
					break;
				}
				else {
...
vector<int> lalian(vector<rbm> bitmap) {
	...
	while (1) {
		...
		for (int i = 0; i < iter.size(); i++) {
			...
			if (i == iter.size() - 1) {
				vector<int> tmp = lalian2(bitmap, iter);
...
int main() {
	vector<string> p(12);
	for (int i = 0; i < 12; i++) {
		p[i] = "./data/1000/test" + to_string(i) + ".txt";
	}
	for (int i = 0; i < 12; i++) {
		string path = p[i];
		fstream f;
